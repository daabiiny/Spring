...
public class AdminInterceptor /* 1 */implements HandlerInterceptor {
	...
	// 2) 메뉴를 클릭했을 떄, 로그인 상태에 따라 컨트롤러의 함수를 실행하지 않아야 하므로 preHandle을 사용한다
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 5) 만약, 로그인 객체가 없거나, 로그인되어있지만 관리자가 아니라면 진행을 중단해야한다
		if(login == null || login.getUserid().equals("admin") == false) {
			...
			String path = "/WEB-INF/view/alert.jsp";
			
			// 8) forward를 수행하기 위한 RequestDispatcher 객체를 불러온다
			RequestDispatcher rd = request.getRequestDispatcher(path);
			
			// 9) forward하기 전에 필요한 값을 setAttribute로 넣어준다()mav.addObject()와동일하다)
