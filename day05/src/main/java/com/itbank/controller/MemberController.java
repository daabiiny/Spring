...
@Controller
@RequestMapping("/member")	// - 전체 경로를 지정하고
public class MemberController {

	@Autowired private MemberService memberService;
	
	@GetMapping("/list")	//"/member/list" - 세부 경로를 지정함
	...
	public ModelAndView member(@PathVariable("idx") int idx) {
		// @PathVariable를 사용한다면 반드시 viewName을 지정한다
		// 요청주소가 제각각이므로, JSP의 이름을 고정시켜야한다
		ModelAndView mav = new ModelAndView("/member/info");
		MemberDTO dto = memberService.getMember(idx);
		mav.addObject("dto", dto);	// ModelAndView에 dto라는 이름으로 넣어라!
		return mav;
	}
	@GetMapping("/login")
	public void login() {}
...
//		return "redirect:/member/" + dto.getIdx(); 이렇게 쓸수도 있지만 밑에꺼 써!
