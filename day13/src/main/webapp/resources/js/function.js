// HTML 문서를 불러오지 않아도 설정할 수 있는 전역 변수 및 함수 정의
   ...
   const getDateStringFromLong = function(num){
		const days = ['일', '월', '화', '수', '목', '금', '토']
		const d = new Date(num)
		let yyyy = d.getFullYear()
		let mm = d.getMonth() + 1	// 0이 1월
		let dd = d.getDate()
		let day = d.getDay()
		if(mm < 10)		mm = '0' + mm
		if(dd < 10)		dd = '0' + dd
...
// 		console.log(tag)
	...
	async function submitHandler(event){	// 이벤트 연결
		...
		const opt = {
			method: 'POST',
			body: JSON.stringify(ob),	// 자바스크립트 객체를 JSON형태의 문자형태로 바꿔준다
			headers: {
