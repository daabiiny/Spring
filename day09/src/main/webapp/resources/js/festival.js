...
async function clickHandler2(){	// 요청해서, 응답받은 내용을 HTML 문자열 형태로 반환
								// JSON 객체의 속성을 활용하여 HTML 태그를 생성한다
	const url = cpath + '/ex02/js'
	const result = await fetch(url).then(resp => resp.json())	// fetch(url) - 지정된 주소로 요청하면 controller에 ex02Js()가 실행됨
	...
	for(let i = 0; i < arr.length; i++){
		const dto = arr[i]
		tag += `<div class="item">`
		tag += `	<div>${dto.UC_SEQ}. ${dto.MAIN_TITLE} (${dto.GUGUN_NM})</div>`	
